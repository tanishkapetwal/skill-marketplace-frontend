...
export class AdminService{
    private apiUrl = 'http://localhost:8081/admin/';
     ...
     res:any=[]
     role: string = ''
     addAdmin(data:listingData)
     {return this.http.post(this.apiUrl+'add-admin',data)}

     addSkills(data:SkillList){
        return this.http.post(this.apiUrl+'add-skills',data)
    }
     getSkills():Observable<Array<Skill>>{
        return this.http.get<Array<Skill>>(this.apiUrl+'skills')
    }
     getAllSkills():Observable<Skill[]>{
        return this.http.get<Skill[]>(this.apiUrl+'skills')
    }
     getAllCustomers():Observable<Array<User>>{
        return this.http.get<Array<User>>(this.apiUrl+'all-customers')
    }
     getCustomers():Observable<User[]>{
        return this.http.get<User[]>(this.apiUrl+'all-customers')
    }
